...
fn main() {
    interrupt::free(|cs| {
        ...
        let mut global_p = cortex_m::Peripherals::take().unwrap();
        let p = nrf51::Peripherals::take().unwrap();

                /* Enable external GPIO interrupts */
        global_p.NVIC.enable(Interrupt::TIMER0);
        global_p.NVIC.clear_pending(Interrupt::TIMER0);
